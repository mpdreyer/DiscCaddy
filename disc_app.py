...
# SCUDERIA LIVERY CSS (ULTIMATE CONTRAST FIX)
st.markdown("""
    <style>
    /* Main Background */
    ...
    /* 1. Sidebar Labels (The text ABOVE the box) -> WHITE */
    section[data-testid="stSidebar"] label {
        color: #ffffff !important;
        font-weight: bold;
    }
    
    /* 2. The Input Boxes & Dropdowns Background -> WHITE */
    ...
    /* 3. The Text INSIDE the boxes (Selected Value) -> BLACK (CRITICAL FIX) */
    div[data-baseweb="select"] span, 
    div[data-baseweb="input"] input,
    div[data-testid="stMarkdownContainer"] p {
        color: #000000 !important; /* Force black text inside white boxes */
    }
    
    /* Exception: Keep sidebar markdown text white (outside boxes) */
    section[data-testid="stSidebar"] div[data-testid="stMarkdownContainer"] p {
        color: #ffffff !important; 
    }
    /* Exception: The selected value in dropdown needs to be black */
...
with current_tab[3]:
    ...
    with st.expander(f"➕ Lägg till ny disc för {target_p}"):
        ...
        with st.form("add_cloud"):
            ...
            sp = c5.number_input("Speed", 0.0, 15.0, float(ai_d.get("Speed", 7.0)))
            gl = c6.number_input("Glide", 0.0, 7.0, float(ai_d.get("Glide", 5.0)))
            tu = c7.number_input("Turn", -5.0, 1.0, float(ai_d.get("Turn", 0.0)))
            fa = c8.number_input("Fade", 0.0, 6.0, float(ai_d.get("Fade", 2.0)))
            
            # Auto-calculate stability text
            stab_val = tu + fa
